...
function getStoryContent(postId) {
  return performQuery(`SELECT photojournalism_pictures_json FROM photojournalism_pictures WHERE post_id="${postId}";`);
}
...
async function parseStoryContent(postId, articleId) {
  try {
    log.info(`ID #${postId} | Parsing story content...`);
    const _storyContent = JSON.parse((await getStoryContent(postId))[0].photojournalism_pictures_json);
    ...
    await eachOfSeries(_storyContent, async (storyItem, index) => {
      ...
      _res.push({
        text: toMarkdown(storyItem.title),
        contentType: 4,
        media: _image._id,
      });
    });

    log.info(`ID #${postId} | Story content parsed.`);
    return [_res, _mediaIds, _storyContent.length * 2];
  } catch (error) {
    log.error(`ID #${postId} | Unable to parse story content: `, error);
    throw error;
  }
}
...
export async function cleanSingleMigration(oldId, newId) {
  try {
    ...
    const _imageRecords = await _oldRecord.media_ids.split(',').filter((item) => item);
    const _imageFileNames = await _imageRecords.map((item) => `${item}.jpeg`);
    return Promise.all([
